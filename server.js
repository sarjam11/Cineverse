...
const {
  ...
  movieSyncQueries, scoringQueries,
} = require("./database");
...
app.post("/api/conversations/:id/messages", authenticate, async (req, res) => {
  try {
    ...
    // Sync movie to recommender engine
    movieSyncQueries.ensureMovie({
      title: movie.title, year: movie.year, genre: movie.genre,
      director: movie.director, imdb_id: movie.imdb_id,
      overview: movie.overview, poster: movie.poster,
      tmdb_id: movie.tmdb_id, media_type: movie.media_type,
    }).catch(err => console.error("[Rec sync]", err.message));
...
// ======================== RECOMMENDER: TMDB ENRICHMENT ========================

async function enrichMovieForRecommender(tmdbId, mediaType) {
  try {
    const mtype = mediaType === "tv" ? "tv" : "movie";
    const data = await tmdbFetch(
      `/${mtype}/${tmdbId}?append_to_response=credits,keywords`
    );

    const genres = (data.genres || []).map(g => g.name);
    const cast = (data.credits?.cast || []).slice(0, 5).map(c => c.name);
    const director = data.credits?.crew?.find(c => c.job === "Director")?.name || "";
    const keywords = mtype === "movie"
    ...
    await dbQuery(`
      UPDATE movies SET
        tmdb_id = $1, genres = $2, cast_names = $3,
        director = COALESCE(NULLIF($4, ''), director),
        keywords = $5, overview = COALESCE(NULLIF($6, ''), overview),
        tmdb_rating = $7, tmdb_popularity = $8,
        original_language = $9, runtime = $10
      WHERE title = $11 AND year = $12
    `, [
      tmdbId, genres, cast, director, keywords,
      data.overview, data.vote_average, data.popularity,
      data.original_language, data.runtime,
      title, year
    ]);
    return true;
  } catch (err) {
    console.error("[Recommender] Enrich failed for TMDB " + tmdbId + ":", err.message);
    return false;
  }
}

async function batchEnrichMovies() {
  const movies = (await dbQuery(
    "SELECT id, title, year, media_type FROM movies WHERE genres = '{}' OR genres IS NULL LIMIT 100"
  )).rows;

  console.log("[Recommender] Enriching " + movies.length + " movies from TMDB...");
  let enriched = 0;
  ...
  for (const movie of movies) {
    try {
      ...
      const search = await tmdbFetch(
        `/search/${mtype}?query=${encodeURIComponent(movie.title)}${movie.year ? "&year=" + movie.year : ""}`
      );
      if (search.results && search.results.length > 0) {
        const tmdbId = search.results[0].id;
        await dbQuery("UPDATE movies SET tmdb_id = $1 WHERE id = $2", [tmdbId, movie.id]);
        await enrichMovieForRecommender(tmdbId, mtype);
        enriched++;
      }
      // Respect TMDB rate limit (40 req/10s)
      await new Promise(r => setTimeout(r, 260));
    } catch (err) {
      // Skip failures silently, will retry next batch
    }
  }

  console.log("[Recommender] Enriched " + enriched + "/" + movies.length + " movies");
}
...
// "For You" — hybrid personal recommendations
app.get("/api/recommendations/for-you", authenticate, async (req, res) => {
  try {
    if (!recommender.isReady) {
      // Fallback to TMDB trending while engine initializes
      const trending = await tmdbFetch("/trending/all/week");
      return res.json({
        recommendations: (trending.results || []).slice(0, 16).map(m => ({
          ...formatMedia(m),
          source: "trending",
    ...
    const recs = await recommender.recommend(req.userId, 20);

    // Enrich with movie details
    const enriched = recs.map(rec => {
      const movie = recommender.movies.find(m => m.id === rec.movieId);
      ...
      return {
        ...
        source: rec.source,
        explanation: buildExplanation(rec, movie),
      };
    }).filter(Boolean);
...
// "Because you liked X" — content-similar movies
app.get("/api/recommendations/similar", authenticate, async (req, res) => {
  try {
    const { title, year } = req.query;
    if (!title) return res.status(400).json({ error: "title param required" });
    const similar = await recommender.similarTo(title, year || null, 12);
    res.json({ similar, sourceMovie: { title, year } });
  } catch (err) {
...
// "Crew Picks" — what friends are watching
app.get("/api/recommendations/crew-picks", authenticate, async (req, res) => {
  try {
    const picks = await recommender.crewPicks(req.userId, 15);
    res.json({ crewPicks: picks });
  } catch (err) {
    console.error("Crew picks error:", err);
    res.status(500).json({ error: "Failed to get crew picks" });
  }
});

function buildExplanation(rec, movie) {
  switch (rec.source) {
    case "cbf":
      return "Matches your taste in " + (movie.genre || "films");
    case "cf":
      return "Loved by viewers with similar taste";
    case "crew":
      return "Popular with your crew";
    case "popularity":
      return "Trending on CineVerse";
    default:
...
initSchema().then(async () => {
  // Initialize recommender engine (runs in background, non-blocking)
  try {
    await movieSyncQueries.backfillMovies();
    await scoringQueries.rebuildAllScores();

    // Enrich movies with TMDB metadata, then init recommender
    batchEnrichMovies().then(() => {
      recommender.init().catch(err =>
        console.error("[Recommender] Init failed:", err.message)
      );
    });
  } catch (err) {
    console.error("[Recommender] Startup error:", err.message);
  }
  ...
  ║  Frontend:   ${(hasFrontend ? "✓ Serving built app" : "✗ Run npm run build").padEnd(28)}║
  ║  TMDB:       ${(TMDB_KEY ? "✓ Connected" : "✗ NOT SET").padEnd(28)}║
  ║  Recommender:✓ Initializing...            ║
  ╚═══════════════════════════════════════════╝
    `);
  });
  ...
  setInterval(async () => {
    try {
      console.log("[Recommender] Periodic refresh...");
      await scoringQueries.rebuildAllScores();
      await recommender.init();
    } catch (err) {
      console.error("[Recommender] Refresh failed:", err.message);
    }
  }, 6 * 60 * 60 * 1000);
});
