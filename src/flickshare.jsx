...
async function fetchSimilarMovies(title, year) {
  const token = localStorage.getItem("fs:token");
  if (!token) return [];
  try {
    const res = await fetch(
      `/api/recommendations/similar?title=${encodeURIComponent(title)}&year=${year || ""}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) return [];
    const data = await res.json();
...
async function fetchCrewPicks() {
  const token = localStorage.getItem("fs:token");
  if (!token) return [];
  try {
    const res = await fetch("/api/recommendations/crew-picks", {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) return [];
    const data = await res.json();
    return data.crewPicks || [];
  } catch { return []; }
}

// ======================== RECOMMENDER COMPONENTS ========================

function ForYouSection({ onMovieClick }) {
    ...
    return (
      <div style={{ marginBottom: 28 }}>
        <div style={{ color: S.text, fontSize: 16, fontWeight: 700, marginBottom: 12,
          ...
          <span style={{ color: S.purple }}>&#10022;</span> For You
        </div>
        <div style={{ color: S.textSec, fontSize: 13 }}>Finding your next watch...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 8,
        ...
        {recs.slice(0, 12).map((movie, i) => (
          <div key={i} onClick={() => onMovieClick && onMovieClick({
            title: movie.title, year: movie.year, genre: movie.genre,
            poster: movie.poster, director: movie.director, rating: movie.tmdb_rating,
            overview: movie.overview, imdb_id: movie.imdb_id, tmdb_id: movie.tmdb_id,
            media_type: movie.media_type,
          })}
            style={{ minWidth: 130, flexShrink: 0, cursor: "pointer",
              borderRadius: S.radius, overflow: "hidden", background: S.bgCard,
              transition: "transform 0.2s, box-shadow 0.2s" }}
            onMouseEnter={e => {
              e.currentTarget.style.transform = "translateY(-4px)";
              e.currentTarget.style.boxShadow = "0 8px 24px rgba(179,136,255,0.2)";
            }}
            onMouseLeave={e => {
            ...
            {movie.poster ? (
              <img src={movie.poster} alt="" style={{ width: "100%", aspectRatio: "2/3", objectFit: "cover", display: "block" }} />
            ) : (
              <div style={{ width: "100%", aspectRatio: "2/3", background: S.bgHover,
                display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon name="film" size={28} color={S.textMuted} />
              </div>
            )}
            <div style={{ padding: "8px 10px" }}>
              <div style={{ color: S.text, fontSize: 12, fontWeight: 600, lineHeight: 1.3,
                display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                {movie.title}
              </div>
              ...
              <div style={{ color: S.purple, fontSize: 10, fontStyle: "italic", marginTop: 4, lineHeight: 1.3 }}>
                {movie.explanation || "Picked for you"}
              </div>
            </div>
          </div>
        ))}
...
function CrewPicksSection({ onMovieClick }) {
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ color: S.text, fontSize: 16, fontWeight: 700, marginBottom: 12,
        display: "flex", alignItems: "center", gap: 8 }}>
        <Icon name="group" size={18} color={S.purple} /> Your Crew is Watching
      </div>
      <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 8,
        scrollbarWidth: "none", msOverflowStyle: "none" }}>
        {picks.map((movie, i) => (
          <div key={i} onClick={() => onMovieClick && onMovieClick({
            ...
            <div style={{ padding: "8px 10px" }}>
              ...
              <div style={{ color: S.purple, fontSize: 11, fontWeight: 600, marginTop: 4 }}>
                {movie.friendCount} crew member{movie.friendCount > 1 ? "s" : ""} liked this
              </div>
            </div>
          </div>
        ))}
...
function SimilarMoviesSection({ movieTitle, movieYear, onMovieClick }) {
  const [similar, setSimilar] = useState([]);

  useEffect(() => {
    if (movieTitle) {
      fetchSimilarMovies(movieTitle, movieYear).then(setSimilar);
    }
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div style={{ color: S.textSec, fontSize: 11, fontWeight: 700, textTransform: "uppercase",
        ...
        Because you liked <span style={{ color: S.purple }}>{movieTitle}</span>
      </div>
      <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 6,
        scrollbarWidth: "none", msOverflowStyle: "none" }}>
        {similar.slice(0, 8).map((m, i) => (
          <div key={i} onClick={() => onMovieClick && onMovieClick({
            title: m.title, year: m.year, genre: m.genre,
            poster: m.poster, director: m.director, rating: m.tmdb_rating,
            overview: m.overview, imdb_id: m.imdb_id, tmdb_id: m.tmdb_id,
            media_type: m.media_type,
          })}
            style={{ minWidth: 90, textAlign: "center", flexShrink: 0, cursor: "pointer" }}>
            {m.poster ? (
              <img src={m.poster} alt="" style={{ width: 90, height: 135, borderRadius: S.radiusSm, objectFit: "cover" }} />
            ) : (
              <div style={{ width: 90, height: 135, borderRadius: S.radiusSm, background: S.bgHover,
                display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Icon name="film" size={22} color={S.textMuted} />
              </div>
            )}
            <div style={{ color: S.text, fontSize: 11, marginTop: 4, lineHeight: 1.2,
              display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {m.title}
            </div>
            <div style={{ color: S.purple, fontSize: 10, fontWeight: 600 }}>
              {m.similarity}% match
            </div>
          </div>
        ))}
          ...
          {loading ? (
            ...
            <>
              ...
              <SimilarMoviesSection
                ...
                onMovieClick={(m) => navigateToMovie(m.title)} />
            </>
          )}
        </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: S.bg }}>
      ...
      <div style={{ padding: "16px 24px 0" }}>
        ...
        {!query.trim() && (
          <>
            {/* Recommender: For You + Crew Picks */}
            <ForYouSection onMovieClick={(m) => pushView({ type: "movie", movie: m })} />
            <CrewPicksSection onMovieClick={(m) => pushView({ type: "movie", movie: m })} />
...
export default function CineVerse() {
  ...
  const loadUserData = useCallback(async (token) => {
    ...
    try {
      const [convosData, watchlistData, watchedData, meData, usersData, listsData, notifCount, activityData, friendsData, friendReqData] = await Promise.all([
                ...
                return matchedUsers.length > 0 ? (
                  <div style={{ marginBottom: 4 }}>
                    ...
                    {matchedUsers.map(u => {
                      ...
                      const isFriend = friends.some(f => f.id === u.id || f.username === u.username);
                      const hasPendingOutgoing = friendRequests.outgoing.some(r => r.user?.id === u.id);
                      const hasPendingIncoming = friendRequests.incoming.some(r => r.user?.id === u.id);
                      return (
                        <button key={u.username} onClick={async () => {
                          if (existingConvo) {
                            ...
                            handleStartChat(u);
                          } else if (!hasPendingOutgoing && !hasPendingIncoming) {
                            // Send friend request
                            try {
                              await api("/friends/request", { method: "POST", body: JSON.stringify({ userId: u.id }) });
                            ...
                            const req = friendRequests.incoming.find(r => r.user?.id === u.id);
                            if (req) {
                          ...
                          <span style={{ color: existingConvo ? S.blue : isFriend ? S.blue : hasPendingOutgoing ? S.textMuted : hasPendingIncoming ? S.green : S.purple,
                            fontSize: 11, fontWeight: 600 }}>
                            {existingConvo ? "Open" : isFriend ? "Chat" : hasPendingOutgoing ? "Requested" : hasPendingIncoming ? "Accept" : "Send Request"}</span>
                        </button>
                      );
                    })}
